  ...
  return (
    <group ref={group}>
      <mesh>
        ...
        <meshStandardMaterial color="#04291e" emissive="#00cc8e" emissiveIntensity={1.6} metalness={0.75} roughness={0.15} transparent opacity={0.92} />
      </mesh>
      <mesh scale={0.72}>
        <icosahedronGeometry args={[0.82, 2]} />
        <meshBasicMaterial color="#00ffb3" wireframe transparent opacity={0.5} />
      </mesh>
      <mesh ref={ringOne} rotation={[Math.PI / 2, 0, 0]}>
        <torusGeometry args={[1.16, 0.012, 8, 96]} />
        <meshBasicMaterial color="#00ffb3" transparent opacity={0.75} />
      </mesh>
      ...
      <pointLight color="#00ffb3" intensity={2.8} distance={5} />
    </group>
  );
}
...
function Node({ node, index }: { node: StudioNode; index: number }) {
  ...
  const nodeColor = index % 2 === 0 ? '#00ffb3' : '#a855f7';
  ...
  return (
    <group ref={ref} position={node.position}>
      ...
      <mesh>
        ...
        <meshBasicMaterial color={nodeColor} transparent opacity={0.12} />
      </mesh>
    </group>
  );
}
  ...
  return (
    <>
      {lines.map((points, index) => (
        <Line key={index} points={points} color={index % 2 === 0 ? '#00ffb3' : '#a855f7'} transparent opacity={0.28} lineWidth={0.8} dashed dashSize={0.08} gapSize={0.12} />
      ))}
      <Line points={[nodes[0].position, nodes[1].position, nodes[3].position, nodes[2].position, nodes[0].position]} color="#00ffb3" transparent opacity={0.15} lineWidth={0.6} />
    </>
  );
}
...
function StudioScene() {
  ...
  return (
    <group ref={group}>
      ...
      <Sparkles count={80} scale={[8, 6, 4]} size={1.2} speed={0.18} color="#00ffb3" opacity={0.45} />
    </group>
  );
}
  ...
  return (
    <CanvasErrorBoundary fallback={fallback}>
      <Canvas dpr={[1, 1.5]} gl={{ antialias: true, alpha: true }} fallback={fallback}>
        ...
        <pointLight position={[3, 3, 4]} color="#a855f7" intensity={1.5} />
        <StudioScene />
